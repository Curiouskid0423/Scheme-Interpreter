...
def scheme_eval(expr, env, _=None):     # Optional third argument is ignored
    ...
    """
    # Evaluate atoms
    if scheme_symbolp(expr): # if it is a symbol -> lookup
        return env.lookup(expr)
    elif self_evaluating(expr): # if number of self-evaluating -> return
        return expr
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS: # if first is a special operator
        return SPECIAL_FORMS[first](rest, env)
    else:
        fn = scheme_eval(first, env)
        check_procedure(fn)
        # if instance then return scheme_eval (other thing)
        if isinstance(fn, MacroProcedure):
            return scheme_eval(fn.apply_macro(rest, env), env) # apply_macro takes in "operands" only
...
def eval_all(expressions, env, tail=True):
    ...
    if expressions == nil: # edge case
        return None
    elif expressions.rest is nil: # base case
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('Length of Formals ({0}) does not match length of Formals({1})'.format(len(formals), len(vals)))
        while formals != nil:
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        lambda_frame = Frame(self.env).make_child_frame(self.formals, args)
        return lambda_frame
...
def do_define_form(expressions, env):
    ...
    target = expressions.first # the symbol we want
    if scheme_symbolp(target): # defining simple variable
        check_form(expressions, 2, 2)
        value = scheme_eval(expressions.rest.first, env)
        env.define(target, value)    # add to the dictionary of this env
        return target
    elif isinstance(target, Pair) and scheme_symbolp(target.first): # defining a lambda function binding
        function_name = target.first
        full_lambda = Pair(target.rest, expressions.rest)
        env.define(function_name,do_lambda_form(full_lambda, env))
        return target.first
    else:
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if clause.first == 'else':
            ...
            test = scheme_eval(clause.first, env) # if not "else", verify if the predicate is TRUE
...
def make_let_frame(bindings, env):
    ...
    formals, vals = nil, nil    # make list of formals and vals
    while bindings != nil:
...
def do_define_macro(expressions, env):
    ...
    check_form(expressions, 2)
    target, body = expressions.first, expressions.rest
    check_formals(target) # check if formal is 1) a list of symbols and 2) if any symbol is repeated.
    check_form(expressions.first, 0) # check is Pair

    func_name, operands = target.first, target.rest # macro can only be a "macro function"
    macro_objects = MacroProcedure(operands, body, env)
    env.define(func_name, macro_objects)
    return func_name
...
class MuProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        """Make a frame that binds my formal parameters to ARGS, a Scheme list
        of values, for a lexically-scoped call evaluated in environment ENV."""
        mu_frame = env.make_child_frame(self.formals, args) # In problem 11, we used actually self.env 
        return mu_frame
